...
DWORD getOffset(const char *libraryName, DWORD relativeAddr) {
    libBase = findLibrary(libraryName);
    if (libBase == 0)
        return 0;
    return (reinterpret_cast<DWORD>(relativeAddr - libBase));
}
extern "C" {
JNIEXPORT jboolean JNICALL
Java_com_dz_offset_FloatingModMenuService_isGameLibLoaded(JNIEnv *env, jobject thiz) {
